...
def main():
    ...
    day = enterbox('Day:\n (Можно указать несколько введя числа в ввиде "7-13", но не больше 10ти)', 'Datelist') # число без нуля в начале
    days_to_print = []
    if '-' in day:
        day = day.split('-')
        foo = [_ for _ in range(int(day[0]), int(day[1])+1)]
        days_to_print.extend(foo)
    else:
        day = int(day)
        days_to_print.append(day)
    ...
    for d in days_to_print:
        ...
        week = day_datelist[-1]
        qwe = '\n\t'.join(
                        [' : '.join([tpl[0], tpl[1]]) for tpl in day_datelist[:-1] ]
                         )
        datelist += '%s  %s\n' % (week, d) + \
                    '\t' + qwe\
                    + '\n\n' if qwe else\
